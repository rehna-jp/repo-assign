...
public class Bills {
    ...
    // A constructor that initializes the `Bills` object with the specified values
    ...
    // A method named `getKilowatts()` that returns the kilowatts of electricity
    // consumed
    ...
    public double waterBill() {
        if (getGallons() <= 50) { // The first 50 gallons of water consumed are free
            return 0;
        } else { // For any additional gallons consumed beyond 50, the rate is GHS 1.05 per
                 // gallon
